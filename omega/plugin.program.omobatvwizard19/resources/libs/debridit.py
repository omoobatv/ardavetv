    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('Debrid Info Saved for {0}'.format(name), level=getattr(xbmc, 'LOGNOTICE', xbmc.LOGINFO))
        ...
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('Debrid Info Restored for {0}'.format(name), level=getattr(xbmc, 'LOGNOTICE', xbmc.LOGINFO))
            except Exception as e:
