    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('Login Data Saved for {0}'.format(name), level=getattr(xbmc, 'LOGNOTICE', xbmc.LOGINFO))
        ...
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('Login Data Restored for {0}'.format(name), level=getattr(xbmc, 'LOGNOTICE', xbmc.LOGINFO))
            except Exception as e:
