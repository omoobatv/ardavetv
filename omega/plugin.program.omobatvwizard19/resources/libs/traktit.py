    ...
    if do == 'update':
        if not user == '':
            try:
                ...
                logging.log('Trakt Data Saved for {0}'.format(name), level=getattr(xbmc, 'LOGNOTICE', xbmc.LOGINFO))
        ...
        if os.path.exists(file):
            ...
            try:
                ...
                user = addonid.getSetting(default)
                CONFIG.set_setting(saved, user)
                logging.log('Trakt Data Restored for {0}'.format(name), level=getattr(xbmc, 'LOGNOTICE', xbmc.LOGINFO))
            except Exception as e:
